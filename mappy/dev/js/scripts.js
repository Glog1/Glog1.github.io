	...
	heightEditor: function () {
		...
		var logWidth = function () {
			if (viewportWidth < 1025) {
				...
				for (let i= 0; i < sibebarItem.length; i++) {
					sibebarItem[i].addEventListener('click', heightMap);
				}

				function heightMap () {
					let header = document.querySelector('.js-header');
					let editorSideBar = document.querySelector('.js-editorSideBar');
					let editorMap = document.querySelector('.js-editorMap');

					let headerHeight = header.offsetHeight;
					let editorSideBarHeight = editorSideBar.offsetHeight;

					let heightSum = headerHeight + editorSideBarHeight + "px";

					let mainHeight = "100vh" - heightSum;
					console.log(mainHeight);
					editorMap.setAttribute("style","height: calc(100vh - " + heightSum + "); height: -webkit-calc(100vh - " + heightSum + ")");
				}
