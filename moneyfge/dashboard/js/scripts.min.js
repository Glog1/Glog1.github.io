...
var App = {
	...
	mainChartStatistic: function () {
			...
			options: {
				...
				tooltips: {
					...
					custom: function(tooltipModel) {
						// Tooltip Element
						var tooltipEl = document.getElementById('statistic-chart-tooltip');

						// Create element on first render
						if (!tooltipEl) {
							tooltipEl = document.createElement('div');
							tooltipEl.id = 'statistic-chart-tooltip';
						...
						if (tooltipModel.body) {
							...
							var innerHtml = '<thead class="b-chart-tooltip__header">';
							...
							bodyLines.forEach(function(body, i) {
								let bodyPart = body.join('').split(':')
								let circleColor = config.data.datasets[i]
								//var colors = tooltipModel.labelColors[i];
								var style = 'background:' + circleColor.backgroundColor;
								style += '; border-color:' + circleColor.borderColor;
								style += '; border-width: 2px';
								//console.log(style)
								var span = '<span style="' + style + '" class="b-chart-tooltip__circle"></span>';
								innerHtml += '<tr class="b-chart-tooltip__item"><td>' + span + bodyPart[0]+' </td><td class="b-chart-tooltip__val">'+bodyPart[1]+'</td></tr>';
							});
							innerHtml += '<tr class="b-chart-tooltip__payments"><td>Payments</td><td class="b-chart-tooltip__val">34</td></tr></tbody>';

							var tableRoot = tooltipEl.querySelector('.js-tooltip');
				...
				scales: {
					...
					yAxes: [{
						...
						gridLines: {
							color: 'rgba(195, 195, 195, .3)',
							zeroLineColor: 'rgba(195, 195, 195, .3)'
						},
						ticks: {
							min: 0,
							max: 2500,
		...
		window.onload = function () {
			var ctx = document.getElementById('statistcChart')
			if (ctx) {
	...
	mainChartPie: function () {
		let ctx = document.getElementById('paymentsStat');
		if (ctx) {
			let paymentsStat = new Chart(ctx, {
				type: 'pie',
				data: {
					labels: [ 'Received', 'Paid off', 'Returned'],
					datasets: [{
						data: [5, 3.5, 1.5],
						backgroundColor: [
							'#00C48C',
			...
			let paymensPieTooltip = document.querySelector('.js-paymens-pie-tooltip'),
				mainPaymStatLab = paymentsStat.data.labels,
				paymentsPieIt = paymentsStat.data.datasets[0]

			mainPaymStatLab.forEach(function (item,i) {
				paymensPieTooltip.innerHTML += `
				<div class="b-paymens-pie-tooltip__item">
						<div class="b-paymens-pie-tooltip__lab-color" style="background-color: ${paymentsStat.data.datasets[0].backgroundColor[i]}"></div>
						<div class="b-paymens-pie-tooltip__title">${item}</div>
						<div class="b-paymens-pie-tooltip__val">${paymentsStat.data.datasets[0].data[i] * 10} %</div>
				</div>`
			})
		}
	...
	mobileMenu: function () {
		...
		let mobileSideBar = document.querySelector('.js-sidebar');
		...
		selectBlock.forEach(function (item) {
			...
			let selectItem = item.querySelectorAll('.b-select__item')
				...
				selectItem.forEach(function (item) {
					item.addEventListener('click', function (event) {
						let target = event.target,
							targVal = target.getAttribute('data-value')
						//console.log(target.closest('.js-select-block').childNodes[0].nodeName)
	...
	currencyList: function () {
		let currencyBlock = document.querySelectorAll('.js-currency-block');

		currencyBlock.forEach(function (item) {

			item.addEventListener('click', function (event) {
				let target = event.target;

				if (target.classList.contains('js-currency-list-btn') || target.parentElement.classList.contains('js-currency-list-btn')) {
					this.classList.toggle('active');
				}
	...
	navigationSubCat: function () {
		let navItem = document.querySelectorAll('.js-navigation .js-sub-cat')
		...
		navItem.forEach(function (item) {
			...
			item.addEventListener('click', function () {
				...
				item.querySelector('.b-navigation__item-link').classList.remove('active')

				this.classList.add('open')
				this.querySelector('.b-navigation__item-link').classList.add('active')
	...
	optionsBlock: function () {
		let optionsBlock = document.querySelectorAll('.js-options-block')

		optionsBlock.forEach(function (item) {

			item.addEventListener('click', function (event) {
				let target = event.target

				if (target.classList.contains('js-options-button') || target.parentElement.classList.contains('js-options-button') ) {
					this.classList.toggle('active');
				}
...
document.addEventListener('DOMContentLoaded', function () {
	App.init(),
	App.mainChartStatistic(),
	App.mainChartPie(),
	App.mobileMenu(),
	App.mainSelect(),
	App.currencyList(),
	App.navigationSubCat(),
	App.optionsBlock()
